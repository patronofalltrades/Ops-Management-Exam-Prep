...
const PRESETS = [
  ...
  { label: 'Customer Service Slow Desk (S=1, CS=1)', S: 1, CA: 1, CS: 1, mu: 8 },
]
...
export default function UtilizationCurve() {
  ...
  const curveData = useMemo(() => {
    ...
    for (let r = 1; r <= 99; r++) {
      const rv = r / 100
      const lq = computeLq(rv, S, CA, CS)
      points.push({ x: r, y: Math.min(lq, 50) })
    }
    return points
  }, [S, CA, CS])
  ...
  const data = {
    datasets: [
      ...
      {
        label: 'Current ρ',
        data: [{ x: rhoSlider, y: 0 }, { x: rhoSlider, y: Math.min(currentLq, 50) }],
  ...
  const options = {
    ...
    scales: {
      x: {
        ...
        min: 0,
        max: 100,
        title: { display: true, text: 'Utilization ρ (%)', color: txColor },
        ticks: { color: txColor, stepSize: 10 },
        grid: { color: bdColor + '60' },
      },
      y: {
        min: 0,
        max: 50,
        title: { display: true, text: 'Avg Queue Length (Lq)', color: txColor },
        ticks: { color: txColor },
        grid: { color: bdColor + '60' },
      },
    },
