...

  {
    ...
    questionSteps: [
      ...
      },
      {
        ...
        steps: [
          { insight: 'Dedicating 1 of the 4 lines to express means ALL 50% of customers who qualify go to that one line. The key insight: express customers have very LOW service variability ($C_S = 0.2$) because small baskets are predictable.', dumb: 'All quick shoppers in one lane with super-consistent service times. The queue barely forms because there are no surprises.', work: '**Step 1: Find parameters for the express line**\nExpress customers = 50% of total = $12 \\times 0.5 = 6$/hr\n$\\lambda_{\\text{express}} = 6$/hr (all go to the 1 dedicated line)\n$t_S = 5$ min → $\\mu = 60/5 = 12$/hr\n$S = 1$ (one cashier)\n\n**Step 2: Find $C_A$ and $C_S$**\n$C_A = 1$ (arrivals are still random → Poisson)\n$C_S = \\sigma_S / t_S = 1/5 = 0.2$ ← this is the game changer!\nExpress baskets are so consistent that $C_S$ is only 0.2 instead of 1.0\n\n**Step 3: Compute $\\rho$**\n$$\\rho = \\frac{6}{1 \\times 12} = 0.5 = 50\\%$$\n\n**Step 4: Compute $L_q$**\n**Formula** $L_q = \\frac{\\rho^{\\sqrt{2(S+1)}}}{1-\\rho} \\times \\frac{C_A^2+C_S^2}{2}$\n\nExponent: $\\sqrt{2(1+1)} = 2$\n$\\rho^2 = 0.5^2 = 0.25$\nVariability factor: $(1^2 + 0.2^2)/2 = (1 + 0.04)/2 = 0.52$ ← nearly halved!\n$$L_q = \\frac{0.25}{0.5} \\times 0.52 = 0.5 \\times 0.52 = 0.26 \\text{ customers}$$\n\nCompare: old $L_q = 0.9$, new $L_q = 0.26$ — a 71% reduction!\n\n**Step 5: Derive $W_q$ and $W$**\n$W_q = 0.26 / 6 = 0.043$ hr $= 2.6$ min (was 18 min!)\n$W = 2.6 + 5 = 7.6$ min (was 30 min!)\n\n**Why such a huge improvement?**\nTwo things worked together:\n1. Lower $\\rho$ (50% vs 60%) — less congestion\n2. Lower $C_S$ (0.2 vs 1.0) — variability factor dropped from 1.0 to 0.52', result: 'Express customers: wait drops from 18 min → 2.6 min, total from 30 → 7.6 min. Both lower utilization AND lower variability contribute.' },
        ],
      },
      {
        question: 'Should you implement the dedicated line?',
        steps: [
          { insight: 'You cannot just look at express customers — you must check what happens to the REGULAR customers who now use only 3 lines instead of 4. Removing express customers changes the average service time for regulars.', dumb: 'You helped the fast shoppers, but did you screw over everyone else? Always check both sides of any operational change.', work: '**Step 1: Find the regular customer service time**\nThe original average of 12 min was a MIXTURE of express (5 min) and regular ($X$ min):\n**Formula** $\\bar{t}_S = p_{\\text{express}} \\times t_{\\text{express}} + p_{\\text{regular}} \\times t_{\\text{regular}}$\nwhere $p$ = proportion of each type\n\n$12 = 0.5 \\times 5 + 0.5 \\times X$\n$12 = 2.5 + 0.5X$\n$X = (12 - 2.5) / 0.5 = 19$ min\n\nRegular customers take 19 min on average — nearly 4x longer than express!\n\n**Step 2: Find parameters for regular lines (3 lines)**\n$\\lambda_{\\text{regular total}} = 12 \\times 0.5 = 6$/hr\n$\\lambda_{\\text{per line}} = 6 / 3 = 2$/hr\n$\\mu = 60/19 = 3.16$/hr\n$$\\rho = \\frac{2}{3.16} = 0.63 = 63\\%$$\n\n**Step 3: The unknown — what is $C_S$ for regular customers?**\nWe know the original $C_S = 1$ for all customers mixed. But we do NOT know $C_S$ for regular customers alone. It could be anywhere from 0.2 (consistent large baskets) to over 1.0 (wildly variable).\n\nThis matters because $C_S$ directly scales the queue:\n$C_S = 0.2$ → variability factor $= (1 + 0.04)/2 = 0.52$ → $W_q \\approx 17$ min\n$C_S = 1.0$ → variability factor $= (1 + 1)/2 = 1.0$ → $W_q \\approx 32$ min', result: null },
          { insight: 'The decision depends on the unknown $C_S$ of regular customers AND on the strategic importance of each segment. The exam wants you to argue both sides.', dumb: 'The answer is never just "yes" or "no." Show the trade-off, state what information you would need, and give a conditional recommendation.', work: '**The trade-off:**\n\n| Metric | Current (all mixed) | Express (dedicated) | Regular (3 lines) |\n|---|---|---|---|\n| $W_q$ | 18 min | 2.6 min | 17-32 min |\n| $W$ | 30 min | 7.6 min | 36-51 min |\n| $\\rho$ | 60% | 50% | 63% |\n\n**Threshold:** If $C_S < 0.32$ for regular customers, their $W_q$ stays ≤ 18 min (not worse than before). Above 0.32, they suffer.\n\n**What you need to decide:**\n1. What is the actual $C_S$ for regular customers? Measure it.\n2. Are express customers more price-sensitive / likely to leave? Then prioritize them.\n3. What is the profit margin per segment? If express customers buy less but come more often, their lifetime value matters.\n4. Could you add a 5th cashier instead? (But the question says "one of the existing lines")', result: 'The decision is conditional. If $C_S$ for regulars is low (< 0.32), implement it — everyone benefits. If $C_S$ is high, express customers gain massively but regulars suffer. The exam rewards you for analyzing BOTH sides and stating what additional data you need.' },
        ],
      },
    ],
    takeaway: [
