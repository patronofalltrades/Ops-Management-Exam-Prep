    ...
    questionSteps: [
      {
        ...
        steps: [
          { insight: 'To audit a policy, first reverse-engineer the implied safety stock from the current ROP, then compare ordering cost vs holding cost. At EOQ, they should be roughly equal.', dumb: 'Work backwards from what the store is currently doing. Figure out their safety stock, then check if ordering and holding costs are balanced. If one is way bigger, the batch size is wrong.', work: '**Step 1: Find the implied safety stock**\n**Formula** $SS = ROP - \\bar{D}_1 \\times VP$\nwhere:\n$SS$ = safety stock (what we want to find)\n$ROP$ = current reorder point (5 kg)\n$\\bar{D}_1$ = avg demand per day (1 kg/day)\n$VP$ = vulnerable period = $LT$ = 4 days (continuous review)\n\n$SS = 5 - 1 \\times 4 = 1$ kg\nThe store keeps only 1 kg as buffer — is that enough? (We will check in Q2.)\n\n**Step 2: Compute total holding cost (also called "maintenance cost")**\n**Formula** Holding $= (SS + Q/2) \\times v \\times i$\n\nThis is called "maintenance cost" in some textbooks — it is the annual cost of keeping inventory in the store. It has two parts:\n\n$SS$ = safety stock = 1 kg (always in store as buffer)\n$Q/2$ = average cycle stock = $10/2 = 5$ kg (inventory swings from 10 to 0, average is 5)\nTotal average inventory $= SS + Q/2 = 1 + 5 = 6$ kg sitting in the store at any time\n\n$v$ = value per kg = 5\n$i$ = annual holding cost rate = 20% (cost of capital — the money tied up in cheese)\n$v \\times i = 5 \\times 0.20 = 1$/kg/yr (it costs 1 per year to hold 1 kg of cheese)\n\nHolding $= 6 \\text{ kg} \\times 1 \\text{/kg/yr} = 6$/yr\n\nThis is tiny — the store spends almost nothing on storage.\n\n**Step 3: Compute annual ordering cost**\n**Formula** Ordering $= (D/Q) \\times S$\nwhere:\n$D/Q$ = orders per year = $365/10 = 36.5$ orders\n$S$ = cost per order (3)\n\nOrdering $= 36.5 \\times 3 = 109.50$/yr', result: 'Ordering (110) is 18x higher than holding (6). At EOQ they should be roughly equal — this batch is way too small!' },
